...
package com.zz.flink.cdc.format;
...
import com.zz.flink.cdc.format.DebeziumJsonDecodingFormat.ReadableMetadata;
...
public final class DebeziumJsonDeserializationSchema implements DeserializationSchema<RowData> {
    ...
    public void deserialize(byte[] message, Collector<RowData> out) throws IOException {
        ...
        try {
            ...
            if (OP_CREATE.equals(op) || OP_READ.equals(op)) {
                payload.setRowKind(RowKind.INSERT);
                payload.setField(2,new BinaryStringData("I"));
                emitRow(row, payload, out);
            } else if (OP_UPDATE.equals(op)) {
                payload.setRowKind(RowKind.UPDATE_BEFORE);
                payload.setField(2,new BinaryStringData("-U"));
                emitRow(row, payload, out);
                payload = copy(payload);
                payload.setRowKind(RowKind.UPDATE_AFTER);
                payload.setField(2,new BinaryStringData("+U"));
                emitRow(row, payload, out);
            } else if (OP_DELETE.equals(op)) {
                payload.setRowKind(RowKind.DELETE);
                payload.setField(2,new BinaryStringData("D"));
                emitRow(row, payload, out);
            } else {
    ...
    private GenericRowData copy(GenericRowData data) {
        GenericRowData result = new GenericRowData(data.getArity());
        for (int i = 0; i < data.getArity(); i++) {
            result.setField(i, data.getField(i));
        }
        return result;
    }
    ...
    private static RowType createJsonRowType(
            ...
            boolean schemaInclude) {
        DataType mapType = DataTypes.MAP(DataTypes.STRING(), DataTypes.STRING());
        DataType payload =
                DataTypes.ROW(
                        DataTypes.FIELD("before", mapType),
                        DataTypes.FIELD("after", mapType),
