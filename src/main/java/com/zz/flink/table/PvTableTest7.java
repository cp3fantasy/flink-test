...
public class PvTableTest7 {
    ...
    public static void main(String[] args) {
        ...
        tEnv.executeSql("create table gender(\n" +
                "    userId VARCHAR,\n" +
                "    gender VARCHAR,\n" +
                "    status VARCHAR\n" +
                ")with(\n" +
        ...
        String createView = "create view view_pv as\n" +
                "select p.pageId,p.userId,g.gender,p.startTime,p.endTime,p.endTime - p.startTime as duration,p.ts,g.status \n" +
                "from pv AS p JOIN gender FOR SYSTEM_TIME AS OF p.procTime AS g\n" +
